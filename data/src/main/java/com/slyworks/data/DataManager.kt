...
constructor(private val mNetworkRegister:NetworkRegister,
    ...
    = Observable.just(query)
        .flatMap {
            when(type){
                SpecificCryptoSearchType.SEARCH ->
                    mRepo1.getSpecificCryptoInfo(query)
                        .toObservable()
                SpecificCryptoSearchType.DETAILS ->
    ...
    = Observable.just(mNetworkRegister.getNetworkStatus())
                .flatMap {
                    if(it){
                        ...
                        mRepo2.getFavorites()
                              .toObservable()
                              .flatMap { it4:List<Int> ->
                                  mRepo2.getData(it4)
                                      .toObservable()
                                      .flatMap flatMap1@ { it5:List<CryptoModel> ->
                                          ...
                                          Observable.just(Outcome.SUCCESS(value = it5))
                                      }
                              }
